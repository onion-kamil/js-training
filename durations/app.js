...
// let [lessonsByAdam, lessonsByPrzemek, lessonsByMarcin] = [];
...
const sortLessons = person => {
  const startsWith = cours.tutors[person].startsWith;

  for (lesson of lessons) {
    if (lesson.name.startsWith(`${startsWith}`)) {
      cours.tutors[person].lessons.push(lesson);
    }
  }
};

const secondsConvertToHHMMSS = function (time) {
  let toHours = Math.floor(time / 3600);
  let toMinutes = Math.floor((time - toHours * 3600) / 60);
  let toSeconds = Math.floor(
    time - (toHours * 3600 + toMinutes * 60)
  );
...
const calcDurations = person => {
  // Extract [MM] and [SS] from format [MM:SS]
  const minutes = [];
  const seconds = [];

  for (lesson of cours.tutors[person].lessons) {
    // Extract [MM] from format [MM:SS] 
    const minute = parseInt(lesson.length.split(":")[0]);
    minutes.push(minute);

    // Extract [SS] from format [MM:SS] 
    const second = parseInt(lesson.length.split(":")[1]);
    seconds.push(second);
  }

  const sumMinutes = minutes.reduce(
    (sum, prev) => {
      return (sum += prev);
    }
  );
  cours.tutors[person].durations.minutesInSeconds = sumMinutes * 60;

  const sumSeconds = seconds.reduce((sum, prev) => {
    return (sum += prev)
  })
  cours.tutors[person].durations.seconds = sumSeconds;

  const allInSeconds = cours.tutors[person].durations.minutesInSeconds + cours.tutors[person].durations.seconds;

  cours.tutors[person].durations.allInSeconds = allInSeconds;
};

const calcPercentages = (partialValue, totalValue, person) => {
  const percent = (((100 * partialValue) / totalValue).toFixed())
  cours.tutors[person].durations.percent = percent;
}

const calcDurationsAllTime = () => {
  // Extract [MM] and [SS] from format [MM:SS]
  const minutes = [];
  const seconds = [];
  ...
  const sumMinutes = minutes.reduce((sum, prev) => {
    return sum += prev;
  })
  cours.durations.minutesInSeconds = sumMinutes * 60;


  const sumSeconds = seconds.reduce((sum, prev) => {
    return sum += prev;
  })
  cours.durations.seconds = sumSeconds;


  const allInSeconds = cours.durations.minutesInSeconds + cours.durations.seconds;
  cours.durations.allInSeconds = allInSeconds;
}

const renderResult = person => {
  const paragraph = document.createElement("p")

  const time = secondsConvertToHHMMSS(cours.tutors[person].durations.allInSeconds);
  const percent = cours.tutors[person].durations.percent;


  paragraph.innerHTML = (`<span class="bold">${person}</span> - ${time} - <span class="bold">${percent}%</span>`)
  document.body.appendChild(paragraph)
}

const calcTimePerson = person => {
  sortLessons(person);
  calcDurations(person);
  calcPercentages(cours.tutors[person].durations.allInSeconds, cours.durations.allInSeconds, person);
  renderResult(person)
};

const renderDurationsAllTime = () => {
  const paragraph = document.createElement("p")
  const time = secondsConvertToHHMMSS(cours.durations.allInSeconds);

  paragraph.innerHTML = (`<span class="bold">Summary: ${time}</span>`)
  document.body.appendChild(paragraph)
}

function totalDuration() {
  calcDurationsAllTime();
  secondsConvertToHHMMSS(cours.durations.allInSeconds)
  renderDurationsAllTime();
  calcTimePerson("Adam");
  calcTimePerson("Przemek");
  calcTimePerson("Marcin");
}
